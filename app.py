...
# =====================================================================================
# DESIGN SYSTEM — "Cahier d'étude" (study notebook atelier)
# Palette : encre marine + laiton doré sur fond parchemin, filets de cahier.
# Typo    : Fraunces (display, empreinte manuscrite/scolaire) + Inter (UI) + JetBrains Mono (chiffres)
# =====================================================================================
CUSTOM_CSS = """
<style>
...
h1, h2, h3 { font-family: 'Fraunces', serif !important; color: var(--navy) !important; letter-spacing: -0.01em; }
...
.hero-title {
  ...
  font-weight: 600;
  margin: 0;
  background: linear-gradient(100deg, var(--navy) 0%, var(--gold) 85%);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  display: inline-block;
}
.hero-sub { color: var(--ink-light); font-size: .98rem; margin-top: .3rem; }
.hero-tag {
...
/* ---------- SIDEBAR ---------- */
[data-testid="stSidebar"] { background: linear-gradient(180deg, var(--navy) 0%, var(--navy-light) 100%); }
[data-testid="stSidebar"] * { color: var(--parchment) !important; }
[data-testid="stSidebar"] h2, [data-testid="stSidebar"] h3 {
  font-family: 'Fraunces', serif !important; color: var(--gold-light) !important;
  border-bottom: 1px solid rgba(232,200,136,0.25); padding-bottom: .4rem;
}
[data-testid="stSidebar"] input, [data-testid="stSidebar"] textarea,
[data-testid="stSidebar"] [data-baseweb="select"] > div {
  background: rgba(255,255,255,0.07) !important;
  border: 1px solid rgba(232,200,136,0.3) !important;
  border-radius: 8px !important;
}
[data-testid="stSidebar"] input:focus { border-color: var(--gold) !important; box-shadow: 0 0 0 2px rgba(184,137,43,0.35) !important; }
[data-testid="stSidebar"] hr { border-color: rgba(232,200,136,0.2) !important; }
[data-testid="stSidebar"] .stButton>button { width: 100%; }

/* ---------- BUTTONS ---------- */
.stButton>button {
  background: linear-gradient(135deg, var(--gold) 0%, #A5761F 100%);
  color: #fff !important; border: none; border-radius: 10px;
  padding: .5rem 1.1rem; font-weight: 600; letter-spacing: .01em;
  transition: all .18s ease; box-shadow: 0 2px 6px rgba(184,137,43,0.28);
}
.stButton>button:hover { transform: translateY(-2px); box-shadow: 0 8px 16px rgba(184,137,43,0.4); filter: brightness(1.05); }
...
.stTabs [data-baseweb="tab"] {
  ...
  font-family: 'Fraunces', serif; color: var(--ink-light); font-weight: 500;
  transition: background .15s ease;
}
.stTabs [data-baseweb="tab"]:hover { background: #fff; }
.stTabs [aria-selected="true"] { background: var(--navy) !important; color: var(--gold-light) !important; border-color: var(--navy) !important; }

/* ---------- SECTION LABELS ---------- */
.section-label {
  font-family: 'Fraunces', serif; color: var(--navy); font-size: 1.1rem;
  margin: 1.3rem 0 .6rem 0; padding-left: .6rem; border-left: 4px solid var(--gold);
}
...
.dash-card {
  ...
  padding: 1rem 1.15rem; box-shadow: 0 1px 3px rgba(27,42,74,0.06);
  transition: box-shadow .2s ease, transform .2s ease;
}
.dash-card:hover { box-shadow: 0 10px 22px rgba(27,42,74,0.12); transform: translateY(-3px); }
...
.dash-card .value.accent { color: var(--gold); }

/* ---------- PILLS ---------- */
.diff-pill {
  display: inline-block; background: rgba(193,102,107,0.1); color: var(--coral);
  border: 1px solid rgba(193,102,107,0.3); border-radius: 999px;
...
.status-badge.todo { background: rgba(184,137,43,0.12); color: var(--gold); border: 1px solid rgba(184,137,43,0.35); }
.status-badge.done { background: rgba(107,143,113,0.15); color: var(--sage); border: 1px solid rgba(107,143,113,0.35); }

/* ---------- QUIZ / BORDERED CONTAINERS — ruled index-card look ---------- */
[data-testid="stVerticalBlockBorderWrapper"] {
  background: repeating-linear-gradient(var(--parchment) 0px, var(--parchment) 27px, var(--rule) 28px);
  border-radius: 12px !important; border: 1px solid var(--rule) !important;
  padding: .3rem .2rem; position: relative; box-shadow: 0 1px 4px rgba(27,42,74,0.07);
  margin-bottom: .9rem;
}
[data-testid="stVerticalBlockBorderWrapper"]::before {
  content: ""; position: absolute; left: 2rem; top: 0; bottom: 0; width: 1px;
  background: rgba(193,102,107,0.35);
}

.q-badge {
  display: inline-flex; align-items: center; justify-content: center;
  width: 1.7rem; height: 1.7rem; border-radius: 50%;
  background: var(--navy); color: var(--gold-light); font-family: 'JetBrains Mono', monospace;
...
/* ---------- CHAT ---------- */
[data-testid="stChatMessage"] { border-radius: 14px; padding: .4rem .3rem; margin-bottom: .3rem; }

/* ---------- DATAFRAME ---------- */
[data-testid="stDataFrame"] { border-radius: 10px; overflow: hidden; border: 1px solid var(--rule); }

/* ---------- MISC ---------- */
hr { border-top: 1px dashed var(--rule) !important; }
[data-testid="stMetric"] { background: #fff; border: 1px solid var(--rule); border-radius: 12px; padding: .8rem 1rem; }
        ...
        for q in st.session_state.quiz:
            with st.container(border=True):
                ...
                st.markdown(
                    ...
                    f'<span style="font-family:\'Fraunces\', serif; font-size:1.05rem; color:var(--navy); font-weight:600;">{q.question}</span>',
                    unsafe_allow_html=True,
                )
